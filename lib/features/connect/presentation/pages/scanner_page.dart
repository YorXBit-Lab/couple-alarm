...
import 'package:couple_note/features/connect/presentation/provider/scanner_notifier.dart';
...
class _QRScannerPageState extends ConsumerState<QRScannerPage>
  ...
  Future<void> _initializeCamera() async {
    ...
    await notifier.initializeCameraAfterBuild();
  }
    ...
    return Consumer(
      builder: (context, ref, child) {
        final isFlashOn = ref.watch(
          qrScannerProvider.select((s) => s.isFlashOn),
        );

        return IconButton(
          onPressed: () => ref.read(qrScannerProvider.notifier).toggleFlash(),
          icon: Container(
            padding: const EdgeInsets.all(6),
            decoration: BoxDecoration(
              color: isFlashOn
  ...
  Widget _buildCameraSwitchButton() {
    return IconButton(
      onPressed: () => ref.read(qrScannerProvider.notifier).switchCamera(),
      icon: Container(
        padding: const EdgeInsets.all(6),
        decoration: BoxDecoration(
          color: Colors.black.withValues(alpha: 0.3),
  ...
  Widget _buildStatusIndicator() {
    return Consumer(
      builder: (context, ref, child) {
        final scanState = ref.watch(qrScannerProvider);
        final String statusText;
        if (scanState.isProcessing) {
          statusText = 'Đang xử lý...';
        } else if (scanState.isScanned) {
          statusText = 'Đã quét thành công';
        } else if (scanState.isCameraReady) {
          statusText = 'Camera đã sẵn sàng';
        } else {
        ...
        } else if (scanState.isScanned || scanState.isCameraReady) {
    ...
    return Consumer(
      builder: (context, ref, child) {
        final isProcessing = ref.watch(
          qrScannerProvider.select((s) => s.isProcessing),
        );
  ...
  void _showManualInput() async {
    ...
    if (result != null && result.isNotEmpty) {
      ref.read(qrScannerProvider.notifier).processManualInput(result);
    }
  }
