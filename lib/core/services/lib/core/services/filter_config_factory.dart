...
class CommonFilterOptions {
  ...
  static List<FilterOption> getTodoStatusOptions() {
    return [
      FilterOption(
        id: 'completed',
        label: TransKeys.complete.tr(),
        icon: Icons.done,
        color: Colors.red,
      ),
      FilterOption(
  ...
  static List<FilterOption> getTodoSortOptions() {
    final basic = getBasicSortOptions();
    basic.insertAll(0, [
      FilterOption(
        id: 'due_date',
        label: TransKeys.low_priority.tr(),
        icon: Icons.event,
      ),
      FilterOption(
        id: 'priority',
        label: TransKeys.high_priority.tr(),
  ...
  static List<FilterOption> getNoteSortOptions() {
    final basic = getBasicSortOptions();
    basic.insertAll(0, [
      FilterOption(id: 'title', label: TransKeys.title.tr(), icon: Icons.title),
    ]);
...
class FilterConfigFactory {
  ...
  static FilterConfig createNoteFilterConfig({List<String>? userTags}) {
    return FilterConfig(
      pageType: PageType.note,
      title: TransKeys.note_filter.tr(),
      availableSections: [
        createSection(
          title: TransKeys.priority_level.tr(),
          type: FilterType.priority,
          options: CommonFilterOptions.getBasicPriorityOptions(),
          allowMultiSelect: true,
        ),
        createSection(
          title: TransKeys.sort_by.tr(),
          type: FilterType.sort,
          options: CommonFilterOptions.getNoteSortOptions(),
      ...
      dateRanges: [DateRangeFilter(type: FilterType.createdDate)],
    );
  }

  static FilterConfig createTodoFilterConfig({List<String>? userTags}) {
    return FilterConfig(
      pageType: PageType.todo,
      title: TransKeys.todo_filter.tr(),
      availableSections: [
        createSection(
          title: TransKeys.status.tr(),
          type: FilterType.status,
          options: CommonFilterOptions.getTodoStatusOptions(),
          allowMultiSelect: true,
        ),
        createSection(
          title: TransKeys.priority_level.tr(),
        ...
        createSection(
          ...
          options: CommonFilterOptions.getTodoSortOptions(),
          allowMultiSelect: false,
        ),
  ...
  static FilterConfig createConfig(
    ...
    switch (pageType) {
      case PageType.note:
        return createNoteFilterConfig(userTags: userTags);
      case PageType.todo:
        return createTodoFilterConfig(userTags: userTags);
